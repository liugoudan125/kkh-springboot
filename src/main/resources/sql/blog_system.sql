...
(
    ...
    `series_id`     char(36)              DEFAULT NULL COMMENT '专栏ID',
...
(
    ...
    `cover_image` varchar(500)          DEFAULT NULL COMMENT '封面图片URL',
    `author_id`   char(36)     NOT NULL COMMENT '创建者ID',
    `status`      varchar(20)  NOT NULL DEFAULT 'active' COMMENT '状态：active-启用，inactive-禁用',
    `created_at`  datetime     NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
    `updated_at`  datetime     NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '最后更新时间',
    PRIMARY KEY (`id`),
...
(
    `id`          char(36)     NOT NULL COMMENT '主键ID',
    `album_id`    char(36)     NOT NULL COMMENT '相册ID',
...
(
    `id`         char(36)    NOT NULL COMMENT '主键ID',
    `board_id`   char(36)    NOT NULL COMMENT '留言板ID',
    `user_id`    char(36)             DEFAULT NULL COMMENT '用户ID（可为空，支持匿名留言）',
...
(
    `id`         char(36)    NOT NULL COMMENT '主键ID',
    `article_id` char(36)    NOT NULL COMMENT '文章ID',
    `user_id`    char(36)    NOT NULL COMMENT '用户ID（可为空，支持匿名评论）',
    `nickname`   varchar(50) NOT NULL COMMENT '评论者昵称',
    `parent_id`  char(36)             DEFAULT NULL COMMENT '父评论ID（用于回复功能）',
